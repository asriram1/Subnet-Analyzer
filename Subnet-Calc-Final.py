...
def subnet_calc():
    try:  # entire code in try except block, where if the ctrl + c combination is pressed at runtime, exception raised by Python and handled with custom message. 
        print ("\n")
        
        #This checks the validity of the IP address
        while True:
            ip_address = input("Enter an IP address: ")
            
            #Checking octets            
            a = ip_address.split('.')# splits the IP address into octets, and stores in list
                        
            if (len(a) == 4) and (1 <= int(a[0]) <= 223) and (int(a[0]) != 127) and (int(a[0]) != 169 or int(a[1]) != 254) and (0 <= int(a[1]) <= 255 and 0 <= int(a[2]) <= 255 and 0 <= int(a[3]) <= 255):
                break
                #since each octet is stored as a string, this is converted into int variable to compare
                #for IP address starting below and including 223 are unicast addresses. 
                #for IP addresses starting with 127 are reseerved for loopback cases. 
                #for IP addresses starting with 169.254 are configured for DHCP that are unable to contact the HTTP server
            
            else: #if above conditions not met, the IP address is not valid
        ...
        while True:
            ...
            if (len(b) == 4) and (int(b[0]) == 255) and (int(b[1]) in masks) and (int(b[2]) in masks) and (int(b[3]) in masks) and (int(b[0]) >= int(b[1]) >= int(b[2]) >= int(b[3])):
                break
            	#for subnet masks as well the length of the list should be 4
            	#The subnet mask should start with 255
            else:
        ...
        mask_octets_padded = [] #an empty list where each octet of subnet mask stored in binary format
        ...
        for octet_index in range(0, len(mask_octets_decimal)):
            ...
            binary_octet = bin(int(mask_octets_decimal[octet_index])).split("b")[1] #converts the string into int, then binary. Then, the b and 0 at the start of the binary are removed.
             
            if len(binary_octet) == 8:
                mask_octets_padded.append(binary_octet)
            
            elif len(binary_octet) < 8:
                binary_octet_padded = binary_octet.zfill(8) #if binary octet does not have 8 bits, then the octet filled with zero until total no. of bits is 8
        ...
        #Obtaining wildcard mask- this is an inverted subnet mask. Can be obtained by subtracting from 255 for each octet. 
        wildcard_octets = []
        ...
        while True:
            ...
            if generate == "y":
                ...
                for indexb, oct_bst in enumerate(bst_ip_address): #enumerate function returns index of an element and the value of an element at same time.
                    #print indexb, oct_bst
                    for indexn, oct_net in enumerate(net_ip_address):
                        #print indexn, oct_net
                        if indexb == indexn:
                            if oct_bst == oct_net:
                                #Is identical octets, then add to the generated_ip list
